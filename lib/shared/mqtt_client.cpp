...
void publishAggregateStatus(bool retained) {
  ...
  if (publishJsonDocToTopic(statusTopic, doc, retained)) {
    _setStatus("publishing agg", 1500);
  }
}
...
bool mqttConnect() {
  ...
  Serial.print("[MQTT] connect result: "); Serial.println(ok ? "success" : "failure");
  Serial.print("[MQTT] mqtt.state(): "); Serial.println(mqtt.state());

  if (!ok) {
    _setStatus("broker conn fail", 2000);
    return false;
  }
  ...
  if (!subOk) {
    _setStatus("cmd sub failed", 2000);
    mqtt.disconnect();
    return false;
  }
